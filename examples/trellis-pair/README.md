...
- **Substrate-derivation discipline by architecture.** Fitter
  and applier both consume `trellis.observation` from the same
...
- `self.children` iteration. Trellis-pair's coordination is
...
2. Each binary's runtime reads `LOTUS_BUS_CONFIG`. UDP
   multicast group joined for `trellis.observation`; NATS
...
- Applier receives each kernel; hot-swaps `current_kernel`.
  ...
  kernel; publishes `Action` on `trellis.action`.
...
The trellis-pair pulls on every substrate primitive at once:
...
runs without surprise. If something is wrong, the trellis-pair
