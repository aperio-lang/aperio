...
//! `crates/hale-codegen/tests/fixtures/examples/` parses
...
fn examples_dir() -> PathBuf {
    // Cargo runs tests with `crates/hale-syntax` as cwd; walk up
...
fn ap_files(dir: &Path) -> Vec<PathBuf> {
    ...
    for entry in fs::read_dir(dir).expect("read examples dir") {
        ...
        } else if path.extension().and_then(|s| s.to_str()) == Some("hl") {
            out.push(path);
        }
    }
    out
...
fn all_examples_parse() {
    ...
    assert!(!files.is_empty(), "no example .hl files found in {}", dir.display());

    let mut failures = Vec::new();
    for path in &files {
        let source = fs::read_to_string(path).expect("read .hl file");
