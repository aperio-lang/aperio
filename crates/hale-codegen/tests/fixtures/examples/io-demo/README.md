...
`std::str::parse_int` (numeric argv) into a single Hale
...
2. Resolve a config path: `$HALE_IO_DEMO_CONFIG_PATH`
   if set, otherwise `/tmp/hale_io_demo_config.txt`.
3. Resolve a log path: `$HALE_IO_DEMO_LOG_PATH` if set,
   otherwise `/tmp/hale_io_demo_log.txt`.
...
```
hale run examples/io-demo/main.hl
nc 127.0.0.1 9876        # in another terminal
cat /tmp/hale_io_demo_log.txt
```

Custom port:

```
hale run examples/io-demo/main.hl 9000
nc 127.0.0.1 9000
```
...
HALE_IO_DEMO_CONFIG_PATH=/tmp/my-config \
  hale run examples/io-demo/main.hl
```
...
- **Stdlib-loci-via-bundled-source** — the Listener's
  declaration lives in `runtime/stdlib.hl`, concatenated to
  this program at codegen time per the m73a mechanism.
- **`if` / `else` / `let mut` reassignment** — ordinary
  Hale surface, exercised against stdlib return values.
...
`crates/hale-codegen/tests/io_demo.rs` builds this example
