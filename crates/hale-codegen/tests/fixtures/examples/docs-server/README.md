...
Phase 5 capstone — a real, end-to-end HTTP server in Hale
...
Composed: an Hale program that does real work — a docs
viewer you can `curl` against — in roughly 200 lines of code.

## Run

```
cargo run --bin hale -- run examples/docs-server/main.hl
# Custom port + docs dir:
cargo run --bin hale -- run examples/docs-server/main.hl 9000 ./spec
# Bounded accepts (useful for tests):
cargo run --bin hale -- run examples/docs-server/main.hl 8080 ./spec 5
```
...
Every Hale invariant gets exercised:
