 ...
 * driver. Attaches an Hale-published SHM ring as a subscriber
 * and validates the payloads.
 *
 * Used by the shm_ring_publish.rs harness: an Hale publisher
...
/* Must match the Hale-side `type Tick { px: Int; sz: Int; }`
 * declared in the test's source. Both fields are i64 (Hale's
...
int main(int argc, char **argv) {
    ...
    /* Retry-attach: the Hale publisher races to create the
     * ring; we wait for it. */
     ...
     * each. The Hale publisher writes Tick{px: i+1, sz:
