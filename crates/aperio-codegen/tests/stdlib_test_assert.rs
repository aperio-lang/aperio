...
//!
//! Each test compiles a small .ap program that exercises one
...
//!
//! These pin the contract Phase 2 ships. Real Aperio-language
//! tests built ON TOP of this layer live in
//! `tests/aperio_self_test.rs`.
...
fn build_and_run(name: &str, source: &str) -> std::process::Output {
    ...
    bin.push(format!("aperio_test_assert_{}", name));
