...
fn build_aperio(name: &str, source: &str) -> std::path::PathBuf {
    ...
    bin.push(format!("aperio_test_listener_multi_{}", name));
...
fn listener_default_callback_handles_one_connection() {
    ...
    let bin = build_aperio("default_cb", &src);

    // Spawn the Aperio Listener; give it a moment to bind.
...
fn listener_user_callback_receives_usable_stream() {
    ...
    let bin = build_aperio("user_cb", &src);
...
fn listener_handles_multiple_connections_in_sequence() {
    ...
    let bin = build_aperio("multi", &src);
