...
fn build_and_run(name: &str, src: &str) -> (String, String, std::process::ExitStatus) {
    ...
    bin.push(format!("aperio_test_io_err_{}_{}", name, std::process::id()));
...
fn read_file_missing_or_raise_panics_at_root() {
    ...
    assert!(
        stderr.contains("IoError") || stderr.contains("Aperio panic"),
        "stderr: {:?}",
        stderr
    );
}
...
fn or_over_non_fallible_path_call_has_clear_diagnostic() {
    ...
    let program = aperio_syntax::parse_source(src).expect("parse");
    let mut bin = std::env::temp_dir();
    bin.push(format!("aperio_test_or_diag_{}", std::process::id()));
...
fn str_bytes_mismatch_diagnostic_suggests_converter() {
    ...
    let program = aperio_syntax::parse_source(src).expect("parse");
    let mut bin = std::env::temp_dir();
    bin.push(format!("aperio_test_str_diag_{}", std::process::id()));
    let err = aperio_codegen::build_executable(&program, &bin).expect_err("should reject");
    let _ = std::fs::remove_file(&bin);
    let msg = format!("{:?}", err);
...
fn bytes_str_mismatch_diagnostic_suggests_converter() {
    ...
    let program = aperio_syntax::parse_source(src).expect("parse");
    let mut bin = std::env::temp_dir();
    bin.push(format!("aperio_test_bytes_diag_{}", std::process::id()));
    let err = aperio_codegen::build_executable(&program, &bin).expect_err("should reject");
    let _ = std::fs::remove_file(&bin);
    let msg = format!("{:?}", err);
...
fn missing_std_prefix_diagnostic_suggests_correction() {
    ...
    let program = aperio_syntax::parse_source(src).expect("parse");
    let mut bin = std::env::temp_dir();
    bin.push(format!("aperio_test_typo_{}", std::process::id()));
    let err = aperio_codegen::build_executable(&program, &bin).expect_err("should reject");
    let _ = std::fs::remove_file(&bin);
    let msg = format!("{:?}", err);
