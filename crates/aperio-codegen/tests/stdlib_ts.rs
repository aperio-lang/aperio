//! m96: Aperio surface for the tree-sitter substrate.
//!
//! `std::ts::*` exposes parse + walk via i64 handles (1-based,
//! 0 = absent / parse failure). These tests build small Aperio
//! programs, run them, and assert on stdout — exercising the
//! full chain: parse → bundle-merge → lower → link with the
//! aperio-ts-shim staticlib → run, with the tree-sitter Go
//! grammar bundled.
//!
//! The shim staticlib must already be built (workspace
//! `cargo build -p aperio-ts-shim`); the codegen link step
//! locates `libaperio_ts_shim.a` under the workspace target
//! dir. CI builds the workspace before running tests, so the
//! ordering is implicit there. For local `cargo test -p
//! aperio-codegen` runs without a prior workspace build, the
//! `aperio-ts-shim` direct dependency in this crate's Cargo
