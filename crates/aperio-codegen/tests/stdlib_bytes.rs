...
fn build_aperio(name: &str, source: &str) -> std::path::PathBuf {
    ...
    bin.push(format!("aperio_test_bytes_{}", name));
...
fn read_bytes_returns_full_length_with_embedded_nuls() {
    ...
    let bin = build_aperio("read_with_nuls", &src);
...
fn read_bytes_on_missing_file_returns_zero_len() {
    ...
    let src = r#"
        fn main() {
            let b = std::io::fs::read_bytes("/tmp/aperio_definitely_does_not_exist_xyz123.bin");
            println("len=", len(b));
        }
    "#;
    let bin = build_aperio("missing_file", src);
    let out = Command::new(&bin).output().expect("run");
    let _ = std::fs::remove_file(&bin);
...
fn println_of_bytes_prints_summary_not_body() {
    ...
    let bin = build_aperio("println_summary", &src);
    let out = Command::new(&bin).output().expect("run");
    let _ = std::fs::remove_file(&bin);
    let _ = std::fs::remove_file(&path);
...
fn stream_send_bytes_ships_full_body_through_tcp() {
    ...
    let bin = build_aperio("send_bytes", &src);
...
fn bytes_round_trips_through_helper_fn() {
    ...
    let bin = build_aperio("round_trip_fn", &src);
    let out = Command::new(&bin).output().expect("run");
    let _ = std::fs::remove_file(&bin);
    let _ = std::fs::remove_file(&path);
    assert!(out.status.success(), "stderr: {}", String::from_utf8_lossy(&out.stderr));
