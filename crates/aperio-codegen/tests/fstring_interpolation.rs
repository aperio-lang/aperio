...
//! as an Aperio expression and lowers the whole literal to
...
fn build_and_run(name: &str, source: &str) -> (String, std::process::ExitStatus) {
    ...
    bin.push(format!("aperio_test_fstring_{}", name));
...
fn fstring_arithmetic_expression_in_interp() {
    // Interp body is sub-parsed as a full Aperio expression, so
...
fn plain_string_with_braces_still_works() {
    ...
    // as literal characters. Several existing .ap sources rely
    // on this (e.g. apps/operational-graph/main.ap printing JSON).
    let src = r#"
        fn main() {
            println("{unchanged}");
        }
