...
//! test verifies that an `hale run` program can invoke the
//! same `std::*` paths that `hale build` already supports.
...
fn fs_file_exists_and_read_file_round_trip() {
    use std::fs;
    let path = std::env::temp_dir().join("hale_interp_parity_test.txt");
    fs::write(&path, "hello hale").unwrap();
...
fn main() {
    let s = std::io::fs::read_file("/no/such/path/hale_interp_parity")
        or report(err);
    println(s);
}
"#;
...
fn fs_write_then_read_round_trip() {
    ...
    use std::fs;
    let path = std::env::temp_dir().join("hale_interp_parity_write.txt");
    let path_str = path.display().to_string();
    let _ = fs::remove_file(&path);
    let src = format!(
        r#"
