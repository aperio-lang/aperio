...
use hale_runtime::{run_bundle_with_bus, run_program, TransportKind};

fn examples_dir() -> PathBuf {
    // CARGO_MANIFEST_DIR is `crates/hale-runtime`; the example
    // fixtures now live under `crates/hale-codegen/tests/
...
fn parse_and_run(rel: &str) -> Result<i32, String> {
    ...
    let program = hale_syntax::parse_source(&source)
        .map_err(|d| {
            d.iter()
...
#[test]
fn hello_world_runs() {
    assert_eq!(parse_and_run("hello-world/main.hl").unwrap(), 0);
}
...
fn locus_with_run_runs() {
    ...
    assert_eq!(parse_and_run("01-locus-with-run/main.hl").unwrap(), 0);
}

#[test]
fn parent_child_runs() {
    assert_eq!(parse_and_run("02-parent-child/main.hl").unwrap(), 0);
}

#[test]
fn closure_test_runs() {
    assert_eq!(parse_and_run("03-closure-test/main.hl").unwrap(), 0);
}

#[test]
fn closure_absorbed_runs() {
    assert_eq!(parse_and_run("03b-closure-absorbed/main.hl").unwrap(), 0);
}
...
fn closure_bubbled_exits_nonzero() {
    ...
    let result = parse_and_run("03c-closure-bubbled/main.hl");
...
#[test]
fn modes_runs() {
    assert_eq!(parse_and_run("04-modes/main.hl").unwrap(), 0);
}

#[test]
fn mutable_counter_runs() {
    assert_eq!(
        parse_and_run("06-mutable-counter/main.hl").unwrap(),
        0
    );
}
...
fn control_flow_runs() {
    assert_eq!(
        parse_and_run("07-control-flow/main.hl").unwrap(),
        0
    );
}
...
fn monotonic_sleep_runs() {
    ...
    assert_eq!(
        parse_and_run("08-monotonic-sleep/main.hl").unwrap(),
        0
    );
}
...
fn functions_runs() {
    assert_eq!(
        parse_and_run("09-functions/main.hl").unwrap(),
        0
    );
}
...
fn stateful_locus_runs() {
    assert_eq!(
        parse_and_run("10-stateful-locus/main.hl").unwrap(),
        0
    );
}
...
#[test]
fn bus_runs() {
    assert_eq!(parse_and_run("05-bus/main.hl").unwrap(), 0);
}

#[test]
fn fitter_applier_demo_runs() {
    assert_eq!(parse_and_run("fitter-applier-demo/main.hl").unwrap(), 0);
}
...
fn bus_runs_under_ringbuffer_transport() {
    ...
    let program = hale_syntax::parse_source(&source).unwrap();
