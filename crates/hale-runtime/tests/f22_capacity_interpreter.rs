//! v1.x-1: F.22 capacity slots under `hale run` (interpreter).
//! Codegen-side coverage lives in
//! `crates/hale-codegen/tests/f22_capacity_*`. This file
//! validates parity — the same .hl source that works under
//! `hale build` also works under `hale run`.
