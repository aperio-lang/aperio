...
pub fn resolve_path(segments: &[&str]) -> Option<Value> {
    match segments {
        ...
        // hale-runtime); same algorithm as the C side.
        ...
        // (runtime/stdlib/bytes_builder.hl). The `__` prefix marks
        ...
        // iris F.9 (2026-05-23): trig surface. Parity with the
        // codegen libm pass-through in lower_stdlib_path_call_expr.
        ["std", "math", "sin"] => Some(Value::Builtin(BuiltinRef {
            name: "std::math::sin",
            func: Rc::new(std_math_sin),
        })),
        ["std", "math", "cos"] => Some(Value::Builtin(BuiltinRef {
            name: "std::math::cos",
            func: Rc::new(std_math_cos),
        })),
        ["std", "math", "tan"] => Some(Value::Builtin(BuiltinRef {
            name: "std::math::tan",
            func: Rc::new(std_math_tan),
        })),
        ["std", "math", "asin"] => Some(Value::Builtin(BuiltinRef {
            name: "std::math::asin",
            func: Rc::new(std_math_asin),
        })),
        ["std", "math", "acos"] => Some(Value::Builtin(BuiltinRef {
            name: "std::math::acos",
            func: Rc::new(std_math_acos),
        })),
        ["std", "math", "atan"] => Some(Value::Builtin(BuiltinRef {
            name: "std::math::atan",
            func: Rc::new(std_math_atan),
        })),
        ["std", "math", "atan2"] => Some(Value::Builtin(BuiltinRef {
            name: "std::math::atan2",
            func: Rc::new(std_math_atan2),
        })),
...
/// Match the codegen-side inline IR exactly so `hale run` and
/// `hale build` agree on the boundary character set.
...
thread_local! {
    ...
    /// CLI before invoking run_bundle (so `hale run script.hl
    /// foo bar` populates this with ["script.hl", "foo",
...
fn std_math_sin(args: &[Value]) -> Result<Value, String> {
    unary_math(args, "sin", f64::sin)
}
fn std_math_cos(args: &[Value]) -> Result<Value, String> {
    unary_math(args, "cos", f64::cos)
}
fn std_math_tan(args: &[Value]) -> Result<Value, String> {
    unary_math(args, "tan", f64::tan)
}
fn std_math_asin(args: &[Value]) -> Result<Value, String> {
    unary_math(args, "asin", f64::asin)
}
fn std_math_acos(args: &[Value]) -> Result<Value, String> {
    unary_math(args, "acos", f64::acos)
}
fn std_math_atan(args: &[Value]) -> Result<Value, String> {
    unary_math(args, "atan", f64::atan)
}
fn std_math_atan2(args: &[Value]) -> Result<Value, String> {
    if args.len() != 2 {
        return Err(format!(
            "std::math::atan2 takes 2 args, got {}",
            args.len()
        ));
    }
