...
fn lt_files(dir: &Path) -> Vec<PathBuf> {
    let mut out = Vec::new();
    if !dir.exists() {
        return out;
    }
    for entry in fs::read_dir(dir).expect("read examples dir") {
        ...
        let path = entry.path();
        if path.is_dir() {
            out.extend(lt_files(&path));
        } else if path.extension().and_then(|s| s.to_str()) == Some("lt") {
            out.push(path);
        }
    }
    out
...
fn all_examples_parse() {
    ...
    let files = lt_files(&dir);
    assert!(!files.is_empty(), "no example .lt files found in {}", dir.display());

    let mut failures = Vec::new();
    for path in &files {
        let source = fs::read_to_string(path).expect("read .lt file");
