//! Integration test for `hale fetch`. Sets up a throwaway
//! "remote" git repo as a `file://` URL, points a manifest at
//! it, runs the `hale` binary's fetch subcommand, and verifies
//! that the source landed under `vendor/<name>/` and that
//! `hale.lock` pinned the resolved SHA.
//!
...
/// with an "hale-test" user identity to avoid relying on the
/// caller's git config.
fn make_repo(dir: &Path, files: &[(&str, &str)]) -> String {
    run(dir, &["init", "--quiet", "--initial-branch=main"]);
    run(dir, &["config", "user.email", "test@hale.invalid"]);
    run(dir, &["config", "user.name", "Hale Test"]);
...
fn fetch_clones_dep_and_writes_lockfile() {
    ...
    // 1. Build a "remote" repo with one .hl file.
    let remote = unique_dir("remote");
    let remote_sha = make_repo(
        &remote,
        &[("greet.hl", "fn hello() -> String { return \"hi\"; }\n")],
    );

    // 2. Build a "consumer" project with an hale.toml pointing
    ...
    fs::write(consumer.join("hale.toml"), manifest).expect("write manifest");

    // 3. Run `hale fetch <consumer>`.
    let out = Command::new(hale_bin())
        .args(["fetch", consumer.to_str().unwrap()])
        .output()
        .expect("spawn hale fetch");
    ...
    assert!(
        cloned.join("greet.hl").exists(),
        "expected {} to exist",
        cloned.join("greet.hl").display()
    );

    // 5. Verify hale.lock has the right SHA.
    let lock = fs::read_to_string(consumer.join("hale.lock"))
        .expect("read lock");
    assert!(
...
fn refuses_to_clobber_hand_maintained_vendor_dir() {
    ...
    let _sha = make_repo(&remote, &[("x.hl", "fn x() { }\n")]);
