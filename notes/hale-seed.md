...
*spell* in Hale's grimoire register) draws from one or
...
Hale's existing register is consistent here:
...
A single seed exists at v0: the Hale standard library.

- **Source location:** `crates/hale-codegen/runtime/stdlib/*.hl`
- **Exposed under:** the magic `std::*` path-call mechanism.
- **Bundling:** every Hale binary compiled by codegen has the
  std seed concatenated into its source via `STDLIB_AP_SOURCE`
  (see `crates/hale-codegen/src/codegen.rs`).
...
User-defined seeds are **not shipped at v0.** An Hale app
today is a single `apps/<name>/main.hl` file that draws only
...
Hale modules milestone:

- **Layout.** A seed is a directory of `.hl` files plus a
  manifest (proposed: `Seed.hl` for an Hale-shaped manifest,
...
- **Compilation.** Each seed compiles independently to an
  ...
  the user's own `.hl` files. Caching at the seed boundary
  becomes possible.
...
dedicated community seeds (`hale-iter`, `hale-tagged`,
`hale-name`, `hale-json`) where they belong.
...
- The `hale-` prefix marks first-party seeds maintained by
  the Hale language project. Examples: `hale-iter`,
  `hale-tagged`, `hale-json`.
...
- **package** — boring but unambiguous; would match Go/Python/
  etc. precedent. Loses Hale's distinctive register entirely.
...
spell. That dormancy maps cleanly onto how Hale compilation
works — the seed is bytes on disk until a program imports it
and the codegen weaves it into the running locus tower.

## Cross-references

- `notes/hale-types-vs-loci.md` — the axiom that constrains
  seed exports to types + loci.
- `spec/styleguide.md` — applies the axiom and the seed
  concept to writing idiomatic Hale.
- `crates/hale-codegen/src/codegen.rs` — `STDLIB_AP_SOURCE`
...
1. **Manifest syntax.** `Seed.hl` (Hale-shaped, parsed by the
   compiler) vs. `seed.toml` (parser-light, separate format).
   Hale-shaped is more in-register; toml is more practical
...
2. **Versioning and compatibility.** How does a seed declare
   ...
   seeds? Is semver appropriate, or does Hale want a richer
...
3. **Re-exports.** Can seed A re-export a type/locus from seed
   ...
   languages allow this; whether Hale does depends on how
...
- `lib/README.md` — domain types shared across MOA apps;
  ...
  question Q18 above.
- `notes/hale-types-vs-loci.md` — the foundational axiom
