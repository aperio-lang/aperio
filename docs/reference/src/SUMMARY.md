# Summary

[Introduction](./introduction.md)
[Behavior index (status of every runtime/stdlib feature)](./behavior-index.md)

# Lexical structure

- [Tokens, comments, literals, identifiers](./lexical.md)

# Types

- [Types — overview](./types/index.md)
  - [Primitives](./types/primitives.md)
  - [Structs and enums](./types/structs-and-enums.md)
  - [Generics](./types/generics.md)
  - [Perspectives and projection classes](./types/perspectives.md)

# Expressions and statements

- [Expressions](./expressions/index.md)
- [Statements](./statements/index.md)

# Loci

- [Locus declarations](./loci/index.md)
  - [Lifecycle methods](./loci/lifecycle.md)
  - [Params](./loci/params.md)
  - [Closures](./loci/closures.md)
  - [Bus blocks](./loci/bus.md)
  - [Fn members](./loci/fn-members.md)

# Bus

- [Bus dispatch and routing](./bus/index.md)

# Closures

- [Closure assertions](./closures/index.md)
  - [Epoch semantics](./closures/epochs.md)
  - [ClosureViolation propagation](./closures/violation.md)

# Recovery

- [Recovery operations](./recovery/index.md)

# Generics

- [Monomorphization](./generics/index.md)

# Memory model

- [Per-region arenas and cross-locus copy semantics](./memory.md)

# Runtime

- [Scheduler, bus router, transport layer](./runtime.md)

# Deployment

- [deployment.yaml schema and env vars](./deployment.md)

# Appendices

- [Grammar (full EBNF)](./grammar.md)
- [Glossary](./glossary.md)
