...
Hale's stdlib intentionally stays narrow — libc + OpenSSL is
...
link surface in an `hale.toml`. Consumers `import` the lib like
...
└── lib/
    └── doubler/
        ├── hale.toml         ← [ffi] declares link surface
        ├── doubler.hl          ← the Hale user surface
        └── glue.c              ← the C-side adapter
```

**`lib/doubler/doubler.hl`** — the Hale surface:
...
**`lib/doubler/hale.toml`** — link surface:
...
**`apps/main/main.hl`** — the consumer:
...
reads `lib/doubler/hale.toml`'s `[ffi]` section automatically.

## What gets wrapped

`@ffi("c") fn name(args) -> ret;` declarations sit at the top
level of a `.hl` file. The compiler emits an LLVM `declare` for
...
| `Bytes` | Hale's `[int64 len][payload]` header pointer | Use `lotus_bytes_len(p)` / `lotus_bytes_data(p)` on the C side to inspect. |
...
ABI is platform-variable), `Uint` (Hale-internal), projections,
fixed-size arrays, tuples, function-pointer types, fallible
types. Cross-boundary errors flow via error sentinels (a
distinguished return value) which the Hale wrapper translates
...
- [`spec/ffi.md`](../../../spec/ffi.md) — the substrate's
  ...
  diagnostic surface.
- [`agents/binding-packages.md`](../../../agents/binding-packages.md)
  — authoring brief. File layout, three-layer Hale surface,
