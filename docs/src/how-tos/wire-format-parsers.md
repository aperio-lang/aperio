...
The Hale stdlib ships flat-JSON (`std::json`) but nothing
...
you write the parser in Hale against the byte-walking
...
1. **Always range-check before reading.** Hale's `std::bytes::at`
