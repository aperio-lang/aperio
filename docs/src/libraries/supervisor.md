<!-- Synced from aperio-lang/pond/supervisor/README.md by tools/sync-pond-docs.sh — do not edit here. -->
...
Erlang OTP's supervision-tree pattern grafted onto Aperio's built-in
...
## Strategies → Aperio recovery primitives
...
- `"transient"` — restart only on abnormal exit
  ...
  In Aperio v1, `on_failure` ONLY runs on ClosureViolation (a clean
  collapse never invokes the handler per F.11), so this currently
  has the same effect as `"permanent"`. The branch is documented
  and ready for the day Aperio adds a clean-exit failure type.
...
`aperio build` on the lib directory alone fails with "program has
