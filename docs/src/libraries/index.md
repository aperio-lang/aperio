<!-- Synced from aperio-lang/pond/README.md by tools/sync-pond-docs.sh — do not edit here. -->

# pond — Aperio contrib libraries

The "non-std std lib." Opinionated, domain-shaped Aperio libraries
...
```toml
# in your app's aperio.toml
[deps]
pond = { git = "https://github.com/aperio-lang/pond", tag = "v0.1.0" }
```
...
use. Each lib lives at its own path under `libraries/`.
...
### Tier 1 — Architecture primitive + Rails-shape web stack

| Path | What it is | Suggested alias |
|------|------------|------|
| `moa/` | Memory Owner Architecture substrate (owner/consumer interfaces, snapshot+delta wiring) | `moa` |
...
| `agent/conversation/` | Conversation locus (MOA owner of history) | `conv` |
...
1. Each lib is one Aperio seed (one directory of `.ap` files;
   F.19 per-directory model).
2. Each lib ships `README.md`, source files, `FRICTION.md`, and
   `examples/<demo>/` with an agent-runnable demo.
3. Public surface is locked in [`CONTRACTS.md`](https://github.com/aperio-lang/pond/blob/main/CONTRACTS.md).
