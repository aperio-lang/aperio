...
Handlers and middleware are Aperio loci that structurally
...
| File | What |
|------|------|
| `types.ap` | `RouteParams`, `Context`, `Response` shapes |
| `interfaces.ap` | `Handler`, `Middleware` structural interfaces |
| `lists.ap` | `@form(vec)` storage loci (methods, patterns, handlers, middleware) |
| `match.ap` | Pattern split + match + path/query extraction |
| `params.ap` | `path_param` / `query_param` free fns |
| `router.ap` | `Router` locus + `NotFound404` default + dispatch chain + free-fn register API |
