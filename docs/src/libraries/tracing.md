<!-- Synced from aperio-lang/pond/tracing/README.md by tools/sync-pond-docs.sh — do not edit here. -->
...
| File | What it holds |
|------|---------------|
| `types.ap`  | `SpanId` + `Span` shape records. |
| `tracer.ap` | `Tracer` locus + `__build_otlp_json` / `__nth_field` / `__remove_row` / `__duration_to_int` helpers. |
| `examples/trace-tree/main.ap` | Demo: outer span + nested inner span + tree printer subscriber. |
...
see — Aperio's codegen-v0 doesn't ship a `--lib` mode). The
