...
> How does a locus declare what it holds, and how does
...
The Hale analogue of `Vec<T>` / `std::vector<T>` / Go slices.
...
The Hale analogue of `Map<K, V>` / `std::unordered_map`. The
...
into one type. But in Hale's substrate, every one of those
