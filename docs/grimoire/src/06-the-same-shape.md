...
- **Modes.** Bulk, harmonic, resolution: one kernel, three
  ...
  projection axis.
- **Perspectives.** Same source compiled into both fitter and
  applier; the type *is* the schema; multi-perspective stability
  before commit. The lotus across processes.
