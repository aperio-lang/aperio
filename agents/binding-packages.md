...
Recommended shape for an Aperio library that binds to an external
...
```
pond/<libname>/
├── aperio.toml          # [ffi] section: link libs + csrc files
├── <libname>.ap         # the Aperio-side user surface
├── glue.c               # the C-side adapter (often thin)
└── README.md            # one-screen intro (optional but helps)
```

For larger bindings, split the surface across multiple `.ap`
files per topic (`window.ap`, `drawing.ap`, `input.ap`, ...);
the per-dir seed model bundles them as one lib. The `.c` glue
can also split if useful (`glue_drawing.c`, `glue_input.c`).
List each csrc in `aperio.toml`'s `csrc = [...]`.
...
- `csrc` — repeatable list of `.c` paths relative to this
  `aperio.toml`'s directory. Each compiles as a translation
...
are rejected; convert at the Aperio side if needed.

### 2. Aperio-side value types (`type Foo { ... }`)

Mirror the C struct's field order + types. The substrate passes
user structs **by pointer** at the boundary, so the Aperio
...
and types; document the layout match at the top of `<libname>.ap`
so future-you doesn't reorder fields without thinking. If the
upstream C library uses smaller integer widths
(`uint8_t` / `int32_t` / `float`), keep the Aperio side as
`Int` / `Float` and let the glue do the narrowing — Aperio's
...
shouldn't touch them — wrap in idiomatic Aperio:
...
- Use Aperio idiomatic naming on the wrapper layer (no `raylib_`
...
1. Takes the Aperio-side ABI (pointers to user structs, `int64_t`
...
// Aperio-side struct shape — MUST match the `type Color`
// declared in raylib.ap. Field order, field types, no padding.
typedef struct {
    int64_t r, g, b, a;
} AperioColor;
...
// Struct passed BY POINTER from Aperio side.
void raylib_clear_background(const AperioColor *c) {
    Color rl = {
...
// return. Aperio caller allocates the slot in its arena.
void raylib_get_mouse_position(AperioVec2 *out) {
...
- **NUL-terminate every String you pass to the upstream lib.**
  Aperio Strings are already NUL-terminated, so this is the
  default — but if you're constructing one from a sliced
  view, make sure the NUL survives.
- **Don't retain Aperio pointers past the call.** Every
  `String` / `Bytes` / struct pointer the Aperio caller passed
  is owned by an Aperio arena; the arena may destroy at
...
- **Return heap-allocated `String` / `Bytes` via the caller's
  ...
  size, align)` (header in `crates/aperio-codegen/runtime/`)
...
### Error sentinels → fallible at the Aperio wrapper
...
Then the Aperio wrapper translates:
...
`pond/raylib/smoketest/main.ap`) that exercises the minimal
surface:
...
`aperio build pond/raylib/smoketest/` should produce a binary
...
- Repo or directory name: `<libname>` matching the upstream
  ...
  `libpq`).
- Aperio alias the consumer picks: their call (`import
...
- `@ffi("c")` symbol names: prefixed with the C library's
  ...
  (`fopen`, `read`), prefix at the Aperio side
...
- **Callbacks.** Aperio's FnPtr doesn't currently cross the FFI
  boundary. If the upstream library demands `void
  (*callback)(int)`, the binding has to manage state at the
  C side (typically via a global registry of Aperio-side
  handlers) and the Aperio surface exposes "subscribe to
  events" rather than "register a callback."
- **vararg fns.** C `printf`-style variadic surfaces have no
  Aperio-side analogue. Bind a fixed-arity wrapper at the C
...
- [`agents/memory-patterns.md`](./memory-patterns.md) — the
  hot-path memory shapes a binding lib's Aperio wrapper may
